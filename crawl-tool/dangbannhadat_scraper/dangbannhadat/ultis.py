...
def getLinkData(obj, meta, link):
    ...
    # json_data["error"] = ' '
    # json_data["auto"] = obj.auto
    json_data["updated_at"] = str(datetime.now().timestamp())
    return json_data
...
def insertOne(json_data, connection, table='app_dangbannhadat_scraper'):
    global one_connection, ignore_links, ignore_contents
    connection = one_connection
    cursor = connection.cursor()
    try:
...
def queryTimes(connection):
  cursor = connection.cursor()
  query = "select type, max(post_time) from app_muabannet_scraper where extract(day from now()::timestamp + interval '7' hour) = extract(day from to_timestamp(post_time) + interval '7' hour) group by type"
  cursor.execute(query)
...
class DangBanNhaDatContent:
  """
  Class đặc tả dữ liệu của một hàng trong bảng mua_ban_content
  """
  def __init__(self, website = "", response = None):
    ...
    self.content["updated_at"] = str(datetime.now().timestamp())
