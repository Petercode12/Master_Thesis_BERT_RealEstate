...
class DatasetProcessor(object):
    ...
    def __generate_sentence(self, id, text, labels):
        ...
        if len(encoded_words) == len(tags):
            ...
            # print([{"content": encoded_word, "tag": tags[i]}
            #       for i, encoded_word in enumerate(encoded_words)])
            print("🚀 ~ file: utils.py ~ line 87 ~ id please check sentence id:", id)
    ...
    def encode_sentence(self, sentence: str, add_special_tokens=False):
        return self.__tokenizer.encode(sentence, add_special_tokens=add_special_tokens)
    ...
    def load_tags(self):
        ...
        with open(os.path.join(__location__,'tags.txt'), 'r') as f:
    ...
    def generate_dataset(self, max_len=None):
        ...
        input_ids = pad_sequences( input_ids, maxlen=max_len, dtype="long", truncating="post", padding="post", \
                                   value=self.__tokenizer.pad_token_id )
        tags = pad_sequences( tags, maxlen=max_len, dtype="long", truncating="post", padding="post", \
                              value=self.__tag2idx[PADDING_TAG] )
...
class PhoBERT(object):
    def __init__(self, labels, config_file_path, pretrained_model_path, model_name='MODEL_PHOBERT_BASE') -> None:
        ...
        self.__annotator = VnCoreNLP(os.path.join(self.__location__,'VnCoreNLP/VnCoreNLP-1.1.1.jar'), annotators="wseg,pos,parse")
        if model_name == 'MODEL_PHOBERT_BASE':
            self.__tokenizer = AutoTokenizer.from_pretrained("vinai/phobert-base", use_fast=False)
        elif model_name == 'MODEL_PHOBERT_LARGE':
            self.__tokenizer = AutoTokenizer.from_pretrained("vinai/phobert-large", use_fast=False)
    ...
    def save_model(self, filename="NER/phobert"):
        torch.save(self.__model, filename)

    def load_model(self, filename="NER/resources/phobert_base"):
