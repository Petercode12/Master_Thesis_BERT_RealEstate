...
from utilsorg import DatasetProcessor, PhoBERT
...
tokenizer.encode("{} {} {}".format(tokenizer.bos_token, tokenizer.eos_token, tokenizer.pad_token), add_special_tokens=False)
...
input_ids, tags, attention_masks = processor.generate_dataset(max_len=max_len)
...
bert = PhoBERT(processor.get_tags(), 
  config_file_path='NER/PhoBERT_base_transformers/config.json',
  pretrained_model_path='NER/PhoBERT_base_transformers/model.bin')

loss_values, validation_loss_values,  accuracy_values, validation_accuracy_values, f1_score_values, validation_f1_score_values = bert.train(train_dataloader, valid_dataloader, learning_rate=3e-5, eps=1e-8, max_grad_norm=1.0, epochs=50)
